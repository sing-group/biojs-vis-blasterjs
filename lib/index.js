...
module.exports = blasterjs = Class(
    /** @lends Biojs.blasterjs# */
    {
        ...
        _displayAlignments : function(evt, self) {
			...
			if(queryLenght != 0){
                            var noOrderAlignments  = getAlignments(e.target.result);
			    var alignments=orderAlignments(noOrderAlignments,false);
                            createAlignmentComparison(alignments, queryLenght, true, true, self);
                            createAlignmentTable(alignments, self);
                            var hsps = alignments[0].hsp.length;
                            createSingleAlignment(alignments[0], self, hsps, 0);
			}else{
			    alert('This file is not a blast default output file.');
			}
                    }
                    r.readAsText(f);
                } else { 
	    ...
	    function orderAlignments(alignments, order){
		var orderedAlignments = [alignments[0]];		
	        for(var i=1;i<alignments.length;i++){
		    var aux = [];
                    var counter = 0;
		    for(var j=0; j<orderedAlignments.length; j++){
		        counter=j+1;
		        if(order && parseFloat(orderedAlignments[j].hsp[0].eValue) <= parseFloat(alignments[i].hsp[0].eValue)){
			    aux.push(orderedAlignments[j]);
		        }else{ 
		            if(!order && parseFloat(orderedAlignments[j].queryCover) >= parseFloat(alignments[i].queryCover)){
			        aux.push(orderedAlignments[j]);
		            }else{ 
				break; 
			    }
                    ...
                    aux.push(alignments[i]);
		    for(var j=counter; j<orderedAlignments.length; j++){
		        aux.push(orderedAlignments[j]);
		    }
		    orderedAlignments=aux;
	        }	
		...
		return getDivColor(colored, colorNb);
            ...
            function createBody(alignments, queryLenght, container, colored, scoring){
                var alignmentContainer = document.createElement('div');
                alignmentContainer.style.paddingBottom = '10px';
                for(var i = 0; i < alignments.length; i++){
                    var alignment = createAlignmentDiv(getColor(colored, scoring, alignments[i].hsp[0].score, alignments[i].hsp[0].eValue), alignments[i], queryLenght);
	    ...
	    function createChangeScoringButton(alignments, lenght, colored, scoring, self){
                ...
                button.onclick=function(){ changeScoring(alignments, lenght, button, colored, scoring, self); };
                return button;
		
	    }		

            function createChangeColorButton(alignments, lenght, colored, scoring, self){
                var button = document.createElement('button');
                ...
                button.style.marginTop   = '5px';
                if(colored == true){
                    var text = document.createTextNode('Change colors to grayscale');
                }else{
                    var text = document.createTextNode('Change colors to full colored');
                }
                button.appendChild(text);
                button.onclick=function(){ changeColors(alignments, lenght, button, colored, scoring, self); };
                return button;
            }
            ...
            function changeColors(alignments, lenght, button, colored, scoring, self){   
                if(colored == true){
                    colored = false;
                }else{
                    colored = true;   
                ...
                createAlignmentComparison(alignments, lenght, colored, scoring, self);
            }

	    function changeScoring(alignments, lenght, button, colored, scoring, self){
                if(scoring == true){
                    scoring = false;
                }else{
                    scoring = true;   
            ...
            function createAlignmentComparison(alignments, queryLenght, colored, scoring, self){
                var blastDiv  = document.getElementById(self.opt.multipleAlignments);
                ...
                var buttonColors        = createChangeColorButton(alignments, queryLenght, colored, scoring, self);
                var buttonScoring       = createChangeScoringButton(alignments, queryLenght, colored, scoring, self);
                ...
                container = createHeader(container, colored, scoring, queryLenght);
                container = createBody(alignments, queryLenght, container, colored, scoring);
                container.appendChild(input);    
