...
module.exports = blasterjs = Class(
    /** @lends Biojs.blasterjs# */
    {
        ...
        _displayAlignments : function(evt, self) {
            if (window.File && window.FileReader && window.FileList && window.Blob) {
			    ...
			    var alignments  = getAlignments(queries[0]);
                            var queryLenght = getQueryLenght(queries[0]);
                            createAlignmentComparison(queries, hsps, 0, alignments, queryLenght, true, true, self);
			    ...
			    alert('This file is not a Blast text output file.');
			}
                    }
                    r.readAsText(f);
                } else { 
	    ...
	    function createNextQueryButton(queries, hsps, position, queryLenght, colored, scoring, self){
                var button = document.createElement('button');
                button.id                = 'newtQuery';
                button.className         = 'btn';
                button.style.marginTop   = '5px';
		button.style.float       = 'right';
                var text = document.createTextNode('Next query');
                button.appendChild(text);
                button.onclick=function(){ nextQuery(queries, hsps, position, queryLenght, colored, scoring, self); };
                return button;
            }

	    function nextQuery(queries, hsps, position, queryLenght, colored, scoring, self){
		 position++;
		 if(position==queries.length){
                     position=0;
                 }
		 var alignments = getAlignments(queries[position]);
                 var queryLenght = getQueryLenght(queries[position]);
                 createAlignmentComparison(queries, hsps, position, alignments, queryLenght, colored, scoring, self);
                 createAlignmentTable(alignments, self);
                 var hsps = alignments[0].hsp.length;
                 createSingleAlignment(alignments[0], self, hsps, 0);
            }
	    ...
	    function getQueryText(content){
            ...
            function createAlignmentComparison(queries, hsps, position, alignments, queryLenght, colored, scoring, self){
                var blastDiv  = document.getElementById(self.opt.multipleAlignments);
		...
		var buttonColors        = createChangeColorButton(queries, hsps, position, alignments, queryLenght, colored, scoring, self);
                var buttonScoring       = createChangeScoringButton(queries, hsps, position, alignments, queryLenght, colored, scoring, self);
                ...
                if(queries.length>1){
                    var text            = document.createElement('div');
		    var nextQueryButton = createNextQueryButton(queries, hsps, position, queryLenght, colored, scoring, self);
                    var divClear        = document.createElement('div');
		    var queryText       = 'Query: '+getQueryText(queries[position]);
		    text.style.color         = '#5C6D7E';
		    text.style.textAlign     = 'center';
		    text.style.paddingBottom = '5px';
		    text.innerHTML           = queryText.bold();
		    divClear.style.clear     = 'both';
		    container.appendChild(nextQueryButton);
		    container.appendChild(text);
		    container.appendChild(divClear);
